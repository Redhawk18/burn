//! [sign (1 bit)] [indicator (variable)] [fraction (variable)]
...
pub(crate) fn encode<B: Backend>(normalized: Tensor<B, 2>) -> Tensor<B, 2, Int> {
    ...
    let abs_val = normalized.clone().abs();

    // Sign bit: contributes 128 to the encoded byte when negative
    let is_negative = normalized.lower_elem(0.0);
    let sign_contrib = is_negative.clone().int().mul_scalar(128);

    // Determine which depth bracket each element falls into
    ...
    let at_d6 = bool_and(is_zero.clone().bool_not(), ge_d5.bool_not());
    ...
    // Indicator bit value per depth
    let indicator = zeros_i
        .clone()
        .mask_fill(at_d0.clone(), 64)
        .mask_fill(at_d1.clone(), 32)
        .mask_fill(at_d2.clone(), 16)
        .mask_fill(at_d3.clone(), 8)
        .mask_fill(at_d4.clone(), 4)
        .mask_fill(at_d5.clone(), 2)
        .mask_fill(at_d6.clone(), 1);

    // Max fraction value per depth = 2^fraction_bits - 1
    let max_frac = zeros_i
        .clone()
        .mask_fill(at_d0.clone(), 63)
        .mask_fill(at_d1.clone(), 31)
        .mask_fill(at_d2.clone(), 15)
        .mask_fill(at_d3.clone(), 7)
        .mask_fill(at_d4.clone(), 3)
        .mask_fill(at_d5.clone(), 1)
        .mask_fill(at_d6.clone(), 0);

    // max_frac as float for the multiplication
    let max_frac_f = zeros_f
        .clone()
        .mask_fill(at_d0, 63.0)
        .mask_fill(at_d1, 31.0)
        .mask_fill(at_d2, 15.0)
        .mask_fill(at_d3, 7.0)
        .mask_fill(at_d4, 3.0)
        .mask_fill(at_d5, 1.0)
        .mask_fill(at_d6.clone(), 0.0);

    // Compute t = (abs_val - lower) / (upper - lower), clamped to [0, 1]
    // Use 1.0 as safe range for zero elements and depth-6 to avoid inf
    ...
    let t = (abs_val - lower)
        .div(safe_range)
        .clamp(0.0, 1.0)
        // Force zero for elements where range was tiny (depth 6, zeros)
        .mask_fill(is_tiny_range, 0.0);

    // fraction = round(t * max_frac_f), clamped to [0, max_frac]
    let fraction_f = t.mul(max_frac_f).round().clamp(0.0, 63.0);
    let fraction_raw = fraction_f.int();

    // Clamp to max_frac (belt and suspenders)
    let over_max = fraction_raw.clone().greater(max_frac.clone());
    let fraction = fraction_raw.mask_where(over_max, max_frac);

    // Assemble: sign_contrib + indicator + fraction
    let encoded = sign_contrib + indicator + fraction;
...
pub(crate) fn decode<B: Backend>(encoded: Tensor<B, 2, Int>) -> Tensor<B, 2> {
    ...
    // Detect true zeros: lower 7 bits == 0
    let lower_bits = encoded.clone().remainder_scalar(128);
    let is_zero = lower_bits.clone().equal_elem(0);

    // Sign from top bit
    let sign_int = encoded.div_scalar(128);
    let sign = sign_int.clone().float().mul_scalar(-2.0).add_scalar(1.0);

    // Determine depth from indicator bit position
    let ge_64 = lower_bits.clone().greater_equal_elem(64);
    let ge_32 = lower_bits.clone().greater_equal_elem(32);
    let ge_16 = lower_bits.clone().greater_equal_elem(16);
    let ge_8 = lower_bits.clone().greater_equal_elem(8);
    let ge_4 = lower_bits.clone().greater_equal_elem(4);
    let ge_2 = lower_bits.clone().greater_equal_elem(2);

    let at_d0 = ge_64.clone();
    let at_d1 = bool_and(ge_32.clone(), ge_64.bool_not());
    let at_d2 = bool_and(ge_16.clone(), ge_32.bool_not());
    let at_d3 = bool_and(ge_8.clone(), ge_16.bool_not());
    let at_d4 = bool_and(ge_4.clone(), ge_8.bool_not());
    let at_d5 = bool_and(ge_2.clone(), ge_4.bool_not());
    ...
    // Indicator value to subtract from lower_bits to isolate fraction
    ...
    // max_frac as float for division
    let max_frac_f = zeros_f
        .clone()
        .mask_fill(at_d0, 63.0)
        .mask_fill(at_d1, 32.0)
        .mask_fill(at_d2, 15.0)
        .mask_fill(at_d3, 7.0)
        .mask_fill(at_d4, 3.0)
        .mask_fill(at_d5, 1.0);

    // t = fraction / max_frac, with safe denominator
    let safe_denom = max_frac_f.clamp_min(1.0);
    let t = fraction.float().div(safe_denom);

    // abs_val = lower + t * (upper - lower)
    let range = upper - lower_f.clone();
    let abs_val = lower_f + t.mul(range);

    // Apply sign and zero out true-zero entries
    sign.mul(abs_val).mask_fill(is_zero, 0.0)
}
